import time
def bounded_knapsack_simplified(weights, values, capacity, limit):
    ...
    for i in range(n):
        for j in range(capacity, weights[i] - 1, -1):
            max_k = min(limit, j // weights[i])  # Максимальное количество предмета i
            for k in range(1, max_k + 1):
                if dp[j - k * weights[i]] + k * values[i] > dp[j]:
                    dp[j] = dp[j - k * weights[i]] + k * values[i]
                    counts[j] = counts[j - k * weights[i]].copy()
                    counts[j][i] = counts[j].get(i, 0) + k
...
def bounded_knapsack_meet_in_the_middle(weights, values, capacity, limit):
    start_time = time.perf_counter()
    n = len(weights)
    half = n // 2
    ...
    max_counts_left = [min(limit, capacity // weights[i]) for i in range(half)]
    for counts in product(*[range(c + 1) for c in max_counts_left]):
        weight = sum(counts[i] * weights[i] for i in range(half))
        value = sum(counts[i] * values[i] for i in range(half))
        if weight <= capacity:
            left.append((weight, value))

    # Генерация всех комбинаций для второй половины
    right = []
    max_counts_right = [min(limit, capacity // weights[i]) for i in range(half, n)]
    for counts in product(*[range(c + 1) for c in max_counts_right]):
        weight = sum(counts[i] * weights[i] for i in range(len(counts)))
        value = sum(counts[i] * values[i] for i in range(len(counts)))
        if weight <= capacity:
...
def bounded_knapsack_brute_force(weights, values, capacity, limit):
    start_time = time.perf_counter()
    n = len(weights)
    max_value = 0
    from itertools import product
    ...
    max_counts = [min(limit, capacity // weights[i]) for i in range(n)]
...
capacity = 28
limit = 5  # Каждый предмет можно взять не более 5 раз
bounded_knapsack_simplified(weights, values, capacity, limit)
bounded_knapsack_meet_in_the_middle(weights, values, capacity, limit)
bounded_knapsack_brute_force(weights, values, capacity, limit)