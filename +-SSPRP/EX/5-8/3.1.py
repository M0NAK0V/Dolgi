...
class AntColonyKnapsack:
    ...
    def construct_solution(self):
        ...
        item_counts = [0] * self.num_items  # Количество выборов каждого предмета
        while True:
            item = self.select_item(available_items, remaining_capacity)
            if item is None:
                break
            if item_counts[item] >= 1:  # Ограничение на количество выборов
