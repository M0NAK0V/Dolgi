...
def knapsack_with_items(weights, values, capacity):
    ...
    for i in range(n, 0, -1):
        if dp[i][j] != dp[i-1][j]:
            packed_weights.append(weights[i-1])
            packed_values.append(values[i-1])
            j -= weights[i-1]
    
    # Вывод результатов
    end_time = time.perf_counter()
    elapsed_time = end_time - start_time
    print("Max value:", dp[n][capacity])
    print("Packed items:")
...
def knapsack_01_meet_in_the_middle(weights, values, capacity):
    start_time = time.perf_counter()
    n = len(weights)
    half = n // 2
    ...
    for mask in range(0, 1 << n):
        ...
        for i in range(n):
            if mask & (1 << i):
                weight += weights[i]
                value += values[i]
                current_weights.append(weights[i])
                current_values.append(values[i])
        if weight <= capacity and value > max_value:
            max_value = value
            packed_weights = current_weights
            packed_values = current_values
    # Вывод результатов
    end_time = time.perf_counter()
    elapsed_time = end_time - start_time
...
knapsack_01_meet_in_the_middle(weights, values, capacity)
knapsack_01_brute_force(weights, values, capacity)
